...
if __name__ == '__main__':
    ...
    saveBaseFolder = '/media/yochin/ModMan DB/ModMan DB/ETRI_HMI/Synthetic Test SceneV2/'
    ...
    k_train_range = []
    for round_ang in range(1, 25, 2):       # rotation angle
        for height_ang in range(1, 14, 2):  # altitute angle
            for dist in range(1, 7, 2):     # distance from object to camera
                for ipr in range(1, 4, 1):  # in-plane rotation
                    k = (ipr - 1) * 1872 + (dist - 1) * 312 + (height_ang - 1) * 24 + round_ang
                    k_train_range.append(k)

    # k is all data
    k_all_range = range(1, 5617)
    k_test_range = list(set(k_all_range) - set(k_train_range))


    rnd.shuffle(k_test_range)
        ...
        for iImg in k_test_range:
            ...
            for iDup in range(numcopy):
                ...
                if find_proper_size == True:
                    ...
                    strImgOnlyname = Obj + '_{0:04d}'.format(iImg) + '_%d'%iDup
