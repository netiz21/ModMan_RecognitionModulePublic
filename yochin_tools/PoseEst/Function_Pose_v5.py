...
def PoseEstimate(img, FeatureDB, CoorDB, ret, init_coord):      #image, left_upper point of boundingBox, ReadDB-FeatureDB, ReadDB-CoorDB, CameraMatrix, CameraDistortion
    ...
    try:
        ...
        for m in xrange(len(matches)):
            tempCoord[:,1] = CoorDB[matches[m][0].trainIdx,0]*1000.
            tempCoord[:,2] = CoorDB[matches[m][0].trainIdx,1]*1000.
            tempCoord[:,0] = CoorDB[matches[m][0].trainIdx,2]*1000.
        ...
        #while  type(inliers) == NoneType or len(inliers)<int(round(len(matches)*1/4)) or tvec[0]>0:
